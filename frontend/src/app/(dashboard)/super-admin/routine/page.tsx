    ...
    <div className="space-y-6 p-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">Schedule classes with real-time teacher & room conflict detection.</p>
        </div>
      ...
      <div className="flex flex-wrap items-center gap-4 rounded-xl border border-border bg-card p-4">
        ...
        <select value={filterBatch} onChange={(e) => setFilterBatch(e.target.value)} className="rounded-lg border border-border bg-background px-3 py-2 text-sm text-foreground outline-none transition focus:border-primary">
          <option value="">All Batches</option>
          {batches.map((b: any) => <option key={b.id} value={b.id}>{b.name}</option>)}
        </select>
        <div className="ml-auto flex items-center gap-3 text-xs text-muted-foreground">
          {SLOT_COLORS.slice(0, 3).map((c, i) => <span key={i} className={`inline-block h-3 w-3 rounded border ${c}`} />)}
        ...
        <div className="overflow-x-auto">
          <div style={{ minWidth: 900 }}>
            <div className="grid border-b border-border" style={{ gridTemplateColumns: "70px repeat(7, 1fr)" }}>
              <div className="border-r border-border bg-muted/20 px-2 py-3 text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">Time</div>
              {DAYS.map(day => (
                <div key={day} className={`border-r border-border bg-muted/20 px-2 py-3 text-center text-xs font-bold uppercase tracking-wider last:border-r-0 ${day === new Date().toLocaleDateString("en-US", { weekday: "long" }).toUpperCase() ? "text-primary" : "text-foreground"}`}>
                  {day.slice(0, 3)}
                </div>
              ))}
            </div>
            {loading ? Array.from({ length: 5 }).map((_, i) => (
              <div key={i} className="grid border-b border-border" style={{ gridTemplateColumns: "70px repeat(7, 1fr)" }}>
                ...
                {DAYS.map(d => <div key={d} className="h-16 border-r border-border px-1 py-1 last:border-r-0"><div className="h-10 animate-pulse rounded bg-muted/30" /></div>)}
              </div>
            )) : HOURS.map(hour => (
              <div key={hour} className="grid border-b border-border last:border-b-0" style={{ gridTemplateColumns: "70px repeat(7, 1fr)" }}>
                ...
                {DAYS.map(day => {
                  ...
                  return (
                    <div key={day}
                      className={`min-h-[56px] border-r border-border px-1 py-1 last:border-r-0 transition-colors ${dragOver === cellKey ? "bg-primary/5" : ""}`}
                    ...
                    >
                      {slots.map((slot: any, si: number) => (
                        <div key={slot.id} draggable
                          className={`mb-0.5 rounded border p-1.5 text-[10px] leading-tight cursor-grab active:cursor-grabbing group relative ${SLOT_COLORS[si % SLOT_COLORS.length]}`}>
                          ...
                          <button onClick={() => handleDelete(slot.id)} className="absolute right-0.5 top-0.5 text-[10px] font-bold opacity-0 transition hover:text-destructive group-hover:opacity-100">✕</button>
                        </div>
                      ))}
                    </div>
                  );
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 p-4 backdrop-blur-sm">
          <div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-2xl border border-border bg-card shadow-2xl">
            <div className="sticky top-0 flex items-center justify-between border-b border-border bg-card px-6 py-4">
              ...
              <button onClick={() => setModal(false)} className="text-muted-foreground transition hover:text-foreground">✕</button>
            </div>
            <form onSubmit={handleSubmit} className="space-y-4 p-6">
              {conflicts.length > 0 && (
                <div className="rounded-lg border border-destructive/30 bg-destructive/10 p-3 space-y-1">
                  {conflicts.map((c, i) => <p key={i} className="text-xs text-destructive">{c}</p>)}
                </div>
              )}
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-muted-foreground">Room *</label>
                ...
                <div>
                  <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-muted-foreground">Start *</label>
                ...
                <div>
                  <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-muted-foreground">End *</label>
              ...
              <div>
                <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-muted-foreground">Branch *</label>
              ...
              <div>
                <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-muted-foreground">Batch *</label>
              ...
              <div>
                <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-muted-foreground">Subject & Teacher *</label>
              ...
              <div className="flex justify-end gap-3 pt-2">
                <button type="button" onClick={() => setModal(false)} className="px-4 py-2 text-sm text-muted-foreground transition hover:text-foreground">Cancel</button>
                <button type="submit" disabled={submitting}
                  className={`rounded-lg px-4 py-2 text-sm font-bold transition disabled:opacity-50 ${conflicts.length > 0 ? "bg-destructive text-white hover:bg-destructive/90" : "bg-primary text-primary-foreground hover:bg-primary/90"}`}>
