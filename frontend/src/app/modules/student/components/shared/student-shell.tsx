...
const navItems = [
  ...
  { label: "Course Contents", href: "/student/course-content", icon: "🗂️" },
  { label: "Routine", href: "/student/routine", icon: "🗓️" },
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (parsedUser.role !== "STUDENT") {
        router.push(parsedUser.role === "TEACHER" ? "/teacher" : "/login");
        return;
      }
      setUser(parsedUser);
    } catch {
          ...
          <div className="flex items-center justify-between px-4 py-3 sm:px-6 lg:px-8">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                type="button"
                className="rounded-full border border-border/70 p-2 text-sm text-muted-foreground transition hover:bg-secondary"
                onClick={() => toast.success("You have 3 new updates")}
              >
                🔔
              </button>
            </div>
